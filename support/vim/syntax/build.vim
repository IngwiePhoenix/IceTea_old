...
syn keyword Logic       and or
syn keyword Statement   include set unset function target input condition requires rule profile auto config display type default cache global value return output allow action warning error notice local continue break all export tag range open close read write
syn keyword Todo        TODO FIXME XXX
syn keyword Type        int string bool float version
syn keyword Constant    null true false file never always important normal hidden autogenerated fileTime fileExists
syn keyword Builtins    files dirs matches replace regex execute unlink exists getMakeDeps toString targets fileName dirName unique
